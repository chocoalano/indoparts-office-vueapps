...
const state = () => ({
    ...
    dialogDelete: false,
    headers: [
        { text: 'Nama akses', value: 'name', sortable: false,},
        { text: 'Base akses', value: 'basepermission', sortable: false,},
...
const actions = {
    index({ state, commit }) {
        return new Promise((resolve) => {
            ...
            $axios.get(`/permission?page=${page}&limit=${itemsPerPage}&sortBy=${sortBy}&sortDesc=${sortDesc}&search=${state.search}`)
                .then((response) => {
    ...
    edit({ state, commit }) {
        return new Promise((resolve) => {
            $axios.get(`/permission/${state.editedIndex}`)
                .then((response) => {
    ...
    update({ state, dispatch }) {
        return new Promise((resolve) => {
            $axios.put(`/permission/${state.editedIndex}`, state.form)
                .then((response) => {
                    resolve(response)
                    dispatch('index')
                })
    ...
    destroy({ dispatch, state }) {
        return new Promise((resolve) => {
            $axios.delete(`/permission/${state.editedIndex}`)
                .then((response) => {
                    resolve(response)
                    dispatch('index')
                })
