<template>
    <v-data-table v-if="$can('read-role') && $can('read-permission')" :headers="headers" :items="desserts" :options.sync="options" :server-items-length="totalDesserts"
        ...
        <template v-slot:top>
            <v-toolbar flat>
                ...
                <v-btn small outlined fab color="danger" @click="deletebatch" v-if="$can('delete-role') && $can('delete-permission')">
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    <template v-slot:activator="{ on, attrs }">
                        <v-btn small outlined fab color="primary" class="mb-2" v-bind="attrs" v-on="on" v-if="$can('create-role') && $can('create-permission')">
                ...
                <v-dialog v-model="dialogDelete" max-width="500px" v-if="$can('delete-role') && $can('delete-permission')">
                    <v-card>
        ...
        <template v-slot:[`item.actions`]="{ item }">
            <v-icon small class="mr-2" @click="editItem(item.role_id)" v-if="$can('update-role') && $can('update-permission')">
                mdi-pencil
            </v-icon>
            <v-icon small @click="deleteItem(item.id)" v-if="$can('delete-role') && $can('delete-permission')">
                mdi-delete
            </v-icon>
        </template>
    </v-data-table>
